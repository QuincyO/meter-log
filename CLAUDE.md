...
- **Writes** go through `doPost` → a `switch` on `body.action`: `addStop`, `addDowntime`, `dispatchRequest`, `updateStop`, `endOfDay`, `previewDailyLog`, `saveTravel`, `saveDay`, `saveEmployee`, `deleteEmployee`, `saveTeam`, `deleteTeam`, `saveCaptain`, `deleteCaptain`, `saveSub`, `deleteSub`. (`previewDailyLog` builds the daily-log PDF on demand without writing a Tracker row — it shares `buildDaySummary` with `endOfDay`. `endOfDay` is **idempotent** on (date, installer): it upserts the Tracker row and replaces that day's Timing rows, so regenerating from `edit.html` never duplicates. `saveTravel` **replaces** that day's per-gap travel deductions — the WO→WO travel-time subtractions, stored as gap-tagged `Downtime` rows only; idempotent, so re-reviewing never duplicates (see ARCHITECTURE.md §"Travel vs delay"). `saveDay` upserts the `Days` bookend-times row. `dispatchRequest` is the Apple Shortcut endpoint that logs a pending `Dispatch` request (time + oldJ). After appending any install carrying an oldJ, `addStop` runs `avgDispatchTime()` — the single matcher — which pairs requests to completed installs, fills the matched `Dispatch` rows, and refreshes the `Metrics` average; `applyDispatchDowntime` then only *reports* a "Requested meter?" (`requestedMeter`) stop's wait, reading its reconciled row for the elapsed time and writing a `DISPATCH` `Downtime` row — falling back to the running average when no request matched (see ARCHITECTURE.md §"Dispatch downtime"). `updateStop` can also correct a stop's clock via `arrivalTime` — it keeps the row's calendar date. `saveTeam` accepts crew two ways: `memberLetters` (`{hNumber: letter}` for installers already in `Employees`) and `newMembers` (`[{name, letter}]` typed onto the boat card) — each new name is auto-created/linked via `ensureEmployeeByName` and folded into `memberLetters` by H number, so a boat can hold any number of crew without pre-adding them in the crew manager.)
- **Reads** go through `doGet` on `?action=`: `day`, `lookup`, `geocode`, `nearby`, `pins`, `tracker`, `timing`, `dispatch`, `avgDispatchTime`, `roster`, `idle`. (`day` also returns `day` = the persisted Departure/Returned bookends and `closed` = whether a Tracker row exists for that installer/date. `idle` returns every WO→WO gap for an installer+date plus any saved deductions, feeding the end-of-day travel-subtraction UI on both `index.html` and `edit.html`. `timing` returns all `Timing` rows; `map.html` averages the WO→WO gaps for the analytics "avg time between meters" metric. `dispatch` returns all `Dispatch` rows; `map.html` averages the matched ones for the "avg dispatch downtime" tile. `avgDispatchTime` is a **pure read** of the stored `Metrics` avg dispatch time; the value is kept fresh by the write path's `avgDispatchTime()` — the single source of truth for dispatch matching, which pairs every requested meter to its completed install in `Stops`, fills the matched `Dispatch` rows, and writes the average to `Metrics`. Not yet wired into any UI.)
...
- **Offline queue (`index.html`).** The capture form is offline-first: writes are pushed onto a `localStorage` queue (`enqueue` → `flush`) and retried when `navigator.onLine`. The service worker (`sw.js`) deliberately lets the POST to the Apps Script URL hit the network and fail when offline so the page's own queue owns retry — do not add the endpoint to the SW cache. `flush()` is **re-entrancy-guarded** (`flushing`) and only removes an item from the queue on a genuine success (`resp.ok` **and** a recognized `{ok|duplicate|flagged}` body); an HTTP error, quota/timeout page, or transient `{ok:false}` is **kept and retried**, so a busy-window failure can't silently drop a stop. Queued append writes (`addStop`/`addDowntime`) carry a **client-generated `id`**; the spine's `idExists()` skips a row whose id was already written, so a timed-out-but-succeeded retry no longer duplicates. (Other writes are upserts/replace and already idempotent.) Mutating `doPost` actions run under a `LockService` script lock so concurrent writes stay atomic.
