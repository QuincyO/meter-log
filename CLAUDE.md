...
- **Reads** go through `doGet` on `?action=`: `day`, `lookup`, `geocode`, `nearby`, `pins`, `tracker`, `roster`, `idle`. (`day` also returns `day` = the persisted Departure/Returned bookends and `closed` = whether a Tracker row exists for that installer/date. `idle` returns every WO→WO gap for an installer+date plus any saved deductions, feeding the end-of-day travel-subtraction UI on both `index.html` and `edit.html`.)
