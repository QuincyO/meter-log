...
const boatLabel = t => 'Boat ' + ((t.boatNumber||'') || '—') + (t.boatName ? (' · ' + t.boatName) : '');

function renderTeams(){
  const box = $('teamList');
  if(!state.teams.length){ box.innerHTML = '<div class="empty" style="padding:4px 2px 10px">No boats yet — tap “New boat”.</div>'; return; }
  box.innerHTML = '';
  state.teams.slice()
...
function teamCard(t){
  ...
  card.className = 'card';
  const title = t.id ? esc(boatLabel(t)) : 'New boat';
  ...
  card.innerHTML = `
    ...
    <div class="cardbody hide" data-role="body">
      <div class="grid3">
        <div><label>Boat number</label><input data-f="boatNumber" class="mono" value="${attr(t.boatNumber)}" placeholder="e.g. 11"></div>
        <div><label>Boat name <span style="font-weight:500;color:var(--ink-soft)">(optional)</span></label><input data-f="boatName" value="${attr(t.boatName)}" placeholder="e.g. Sea Ray"></div>
        <div></div>
      </div>
      <div class="grid2">
        <div><label>Captain</label>${nameSelect('captain', t.captainName, state.captains)}</div>
        <div><label>Sub / subforeman</label>${nameSelect('sub', t.subName, state.subs)}</div>
      </div>
      ...
      <div class="actions">
        <button class="btn primary" data-act="save">Save boat</button>
  ...
  card.querySelector('[data-act="save"]').onclick = async () => {
    ...
    if(!g('boatNumber')){ toast('Boat number required'); return; }
    try{
      await post({ action:'saveTeam', id:t.id||undefined,
        boatNumber:g('boatNumber'), boatName:g('boatName'),
        captainName:resolveName('captain'), subName:resolveName('sub'),
        memberLetters:Object.assign({}, assigned),
        newMembers:pending.map(p => ({ name:p.name, letter:p.letter })) });
      toast('Boat saved ✓'); await load();
    } catch(err){ toast(err.message); }
  };

  card.querySelector('[data-act="del"]').onclick = async () => {
    if(!t.id){ card.remove(); return; }           // discard an unsaved card
    if(!confirm(`Delete ${boatLabel(t)}? This removes the boat, not the crew.`)) return;
    try{ await post({ action:'deleteTeam', id:t.id }); toast('Boat deleted ✓'); await load(); }
    catch(err){ toast(err.message); }
  };
...
$('newTeam').onclick = () => {
  const blank = { id:'', boatNumber:'', boatName:'', captainName:'', subName:'', memberLetters:{} };
