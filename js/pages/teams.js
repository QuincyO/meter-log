...
function setMode(m){
  ...
  $('teamsSub').textContent = land
    ? 'One card per sub foreman — a land crew is just that sub’s installers (no crew number). Add people here, or let them add themselves by picking their sub in the capture app’s Settings.'
...
const boatLabel = t => teamType(t)==='land'
...
function renderTeams(){
  ...
  mine.slice()
    .sort((a,b)=> land
      ...
      : String(a.boatNumber||'').localeCompare(String(b.boatNumber||''), undefined, {numeric:true}))
...
function teamCard(t){
  ...
  card.querySelector('[data-act="save"]').onclick = async () => {
    ...
    if(!land && !g('boatNumber')){ toast('Boat number required'); return; }
    if(land && !resolveName('sub')){ toast('Pick a sub foreman'); return; }
    try{
