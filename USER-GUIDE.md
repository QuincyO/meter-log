...
The projection uses real numbers off your day: how long your stops have actually been taking today, and how far the rest of the route still has to drive at the speed you've actually been driving. **A long hold-up doesn't wreck it.** It reads your *typical* stop today, not your average one, so a morning where you waited three hours for a meter costs you that one gap instead of making every remaining stop look like it takes an hour and a quarter. If you logged the wait as downtime it's taken off as well, but you don't have to — it works either way. The driving speed gets the same treatment: walking around at a meter used to count as "driving" and drag your speed down to something no truck does, which made the rest of the route look hours longer than it is. It also refreshes what's been logged from the sheet every few minutes, so it stays right on a phone you're only driving by while someone else logs the meters.
