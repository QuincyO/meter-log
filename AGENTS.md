...
- **Never bump the `sw.js` `CACHE` version. Don't do it, and don't propose it.** This is
  the owner's standing decision (2026-08-14), not a per-change judgement call to re-open:
  **updates to the field are performed manually, by the installer, via Settings ▸ ⟳ Force
  update from GitHub.** A bump makes every phone re-cache the whole shell at once on its
  next open, unasked, on whatever signal it happens to have — the owner wants that moment
  chosen by the person holding the phone. Leave the constant alone even when a change
  "obviously" wants to reach the fleet now; say what shipped and let them press the button.
  **The consequence you must design around:** a phone can sit on old bytes indefinitely and
  by choice, so a new `js/*.js` running against an older `index.html` is not a brief
  post-deploy window — it is an ordinary, long-lived state. That makes defensive binding of
  freshly-added ids a standing requirement rather than a one-release courtesy; see
  §"an element in a page's markup is a CROSS-FILE CONTRACT" below, and note that ⟳ Force
  update is itself a button on the capture page, so code that can unbind it takes the only
  rollout mechanism with it.
...
land routes**. Both modes are live. The capture app **defaults to land** and the
Boat/Land switch sits in **Settings ▸ Work mode** rather than the top bar — it was taken
off the bar in July 2026 because a mis-tap silently retags a whole day's writes, and it
stayed off when boat work came back. The teams admin page keeps its own boat/land *view*
switch on a separate key; see ARCHITECTURE.md §"Work modes (boat / land)". There is **no build step, no package manager, and no framework** — it is a set
...
- `store.js` — `store` (localStorage) + `cfg()`. `idb.js` — the IndexedDB wrapper + `DB_VERSION`. `api.js` — `apiGet(action, params)` / `apiPost(body)` (inject token + URL). `work-mode.js` — `workMode()` + `setWorkMode()`, the capture app's boat/land mode, on the `captureWorkMode` key (**not** the abandoned legacy `workMode`) and defaulting to land; DOM-free so `node --test` can import it. A **hard import** of both `pages/capture.js` and `drive-recorder.js`, so it belongs in the `sw.js` SHELL. Its header carries the reason for the key rename — read it before touching the mode anywhere.
- `queue.js` — offline queue (`enqueue`/`flush`/`paint`/`migrateLegacyQueue`; page UI side-effects via `setQueueHooks`). `activity-log.js` — the status pill's **activity log**: tap the pill for the Sync activity sheet (what is waiting to send, what is running, ~2 days of history, every sent field on tap). Loaded lazily by `capture.js`, deliberately outside `sw.js` SHELL, in its own IndexedDB database — see §"The status pill's activity log" before touching any of that. `daycache.js` — optimistic/reconcile/merge + retention + recent-days. `geocode.js` — addrCache + `resolveAddress` + `backfillAddresses`.
...
- **"Today" and "the day being planned" are two different dates.** The phone can
  ...
  **plan day** rather than `localDate()`. Four traps:
  ...
  (2) **the lock's date must be the plan day** — lock an evening-built plan under
  today's date and the date-equality releases it at midnight, destroying exactly the
  work this allows;
...
- **A field the route reads live must be persisted live — and storing it is only half
  ...
  The second half is the target box, and it used to be **asymmetric**. Day 1 was sized
  ...
  **raising** it did nothing at all: the frozen set was re-committed only when
  `needsCommit` saw an explicit re-plan *and* a changed target. "They get stuck on a
  different value and never got updated" was literally true for every increase.
  All of that machinery is gone with the work-list lock (see the lock bullets below),
  and what replaced it is simpler in both states. **Unlocked**, nothing is frozen, so
  the box only has to ask for a re-chunk: `applyTodayAnchor({ rechunk: true })`, which
  passes `day1Count: null` and sizes every day by the new number. It is the **only**
  caller that asks — Optimize rewrites every `day` tag at the new target moments before
  it calls, so holding the tags there *is* the fresh split, while the box has re-solved
  nothing and its tags still describe the old number. **Locked**, the box is read-only
  and the handler reverts a typed value to the number the day was settled at, with a
  toast saying to unlock. That third answer is the one the old control never had: it
  either worked or silently did nothing, and the installer could not tell which.
  Two things about the read-only half: `readOnly` is the *affordance* only — a number
  input's spinner and a date input's picker both ignore it on some browsers, so the
  `onchange` handlers are the actual guard — and it takes the **plan-date picker** with
  it, because changing the day would release the lock by the date-equality rule, which
  is right when the day rolls on its own and wrong when it happens under a finger.
  The box passes no `travel`, so the re-split falls back to `estimateTravel` and flags
  `wlTimesEstimated` (the route view's "(est.)"); only Optimize has a real matrix.
...
- **The day-1 clock is an ETA input and must never become a sizing input.**
  ...
  *morning* anchor, so an Optimize staged there keeps 08:15 however late it is
  ...
  new name — see the meters/day bullet above. (Nothing sizes a day from meters
  installed any more; `dayCapacity` is gone entirely.);
...
- **No day-average speed prices the route. There is no floor that makes one work.**
  `routeTravel` used to price the remaining legs at `liveMetrics().avgMovingSpeed`.
  That average is distance ÷ (elapsed − idle) with "idle" at or under
  `IDLE_SPEED_MS` = **1.8 km/h**, and the recorder runs whenever the PWA is open,
  *including while the crew is on foot at a meter* — so dooryard minutes and GPS
  jitter land in the numerator as "moving". A `speed > 1` m/s guard (3.6 km/h)
  caught only a phone that never moved; a rural day driven at 60–80 reported
  **18 km/h**. `MIN_BELIEVABLE_SPEED_MPS` was added at 25 km/h to catch that, **and
  it did not**, because the number that broke the 2026-07-31 route was **28**.
  That day is the whole argument and is pinned in `tests/estimate.test.mjs`. At 12:31
  with 13 done the card read *"~13 installs by 3:45 · 8 STOPS SHORT · Route done
  ~5:36 pm"* — projecting **zero** of the eight stops ahead over three and a quarter
  hours, beside a stop card reading *"Arrive 12:36 · in 5 min"*, while the route's own
  `scheduledEta` chain landed the last of them at **14:45**. The crew's 13th was an
  off-plan walk-up, so `done` rose while the route ahead did not change by a metre — a
  controlled experiment nobody meant to run — and remaining travel went **196 → 241
  min on an unchanged 110.8 km**, purely because the tile drifted 34 → 28 while they
  covered 0.2 km in ~21.6 "moving" minutes. **The feedback loop ran backwards: the
  longer the crew spent installing, the fewer installs the gauge promised.**
  Raising the floor would only move the number that defeats it. Effective
  door-to-door speed climbs steeply with leg length — that morning averaged 2.2 km a
  leg around a village, the afternoon 13.9 with a 58.7 km highway haul in it — so one
  number cannot be right at both ends, however it is chosen. Travel comes from
  `estimateTravel`'s per-leg duration ladder now, the same one every other ETA on the
  screen is built from. **Do not "fix" this in `avgMovingSpeed` itself** — raising
...
- **The travel/on-site split is only honest while ONE model prices both halves.**
  ...
  `NOMINAL_TRAVEL_MIN`, floored at `MIN_ONSITE_MIN` — ten minutes taken off morning
  legs really driven in two or three, then the floor putting it back. It arrived at 8
  by two mistakes cancelling when the truth was near 14 (and the crew's own fitted
  `InstallerMetrics.landOnSiteMin` said 11), so every remaining stop was credited with
  six minutes it did not have. A morning netted with one number and an afternoon
  priced with another is two models of one day, and the gauge reads the difference
  between them as pace. Both halves take the **same** lookup now: `paceContext` builds
  it once and hands it to `routeTravel` and `onsitePerStopReal` alike, and
  `nettedGapMin` subtracts each gap's own measured drive rather than a constant. Its
  frame (`paceRouteItems`) must carry the **done** orders as well as the pending ones —
  a `travelLookup` answers only for ids that were in the frame it was built from, so a
  pending-only frame cannot price the legs already behind the crew, and building a
...
- **A second device logs nothing, so nothing on it invalidates the day cache.**
  ...
  **before** `applyTodayAnchor` (a refresh after it would re-time the day off the
...
- **Offline queue mechanics (`js/queue.js`).** Writes are appended to the IndexedDB `queue` store (`enqueue` → `flush`) and retried when `navigator.onLine`. The store's auto-increment `_seq` key gives FIFO order (head = `q[0]`); `_seq` is internal and stripped before the POST. The queue is page-agnostic — a page registers UI side-effects (the duplicate/conflict notice) via `setQueueHooks`. The service worker (`sw.js`) deliberately lets the POST to the Apps Script URL hit the network and fail when offline so the queue owns retry — do not add the endpoint to the SW cache. `flush()` is **re-entrancy-guarded** (`flushing`) and only deletes an item on a genuine success (`resp.ok` **and** a recognized `{ok|duplicate|flagged}` body); an HTTP error, quota/timeout page, or transient `{ok:false}` is **kept and retried**. **A permanently-rejected write no longer wedges the FIFO.** flush() used to stop at the first non-delivered item, so one write the spine *always* rejects (a non-transient `{ok:false}` — an unknown/legacy action, a validation like OTHER-downtime-needs-a-note, a malformed body) stalled every write queued behind it forever. `classifyFlush` (pure, in `js/queue-policy.js`, unit-tested) now sorts *transient* from *definitive*: a definitive reject is retried `MAX_FLUSH_TRIES` (6) times then **parked** (`_parked` flag) so the rest of the queue keeps draining. Parked items are never dropped — they stay in the durable queue, are surfaced (the pill's “N stuck”, plus a notice), and are un-parked + retried from the **Stuck uploads** sheet (`retryParked`) — tapping the status pill opens the activity log, whose Stuck banner leads there (or, when the log isn't loaded, the pill opens Stuck uploads directly). Lock contention (`busy, retry`) and HTTP/transport failures are transient and never counted toward parking. The pill now also tells a genuinely-offline phone from a spine that isn't answering — `navigator.onLine` for the *label only* (it's reliable when true, so a stuck-false reading only keeps the old “offline” wording, never a wrong “online”); the queue behaviour still runs off the real last-flush outcome, not `navigator.onLine`. Queued append writes (`addStop`/`addDowntime`) carry a **client-generated `id`**; the spine's `idExists()` skips a row whose id was already written, so a timed-out-but-succeeded retry no longer duplicates. (Other writes are upserts/replace and already idempotent.) Mutating `doPost` actions run under a `LockService` script lock so concurrent writes stay atomic; `previewDailyLog` (the one POST that writes nothing) deliberately skips the lock so EOD-prefetch traffic never queues behind the crew's real writes. Adding an IndexedDB store means bumping `DB_VERSION` in `js/idb.js` and adding it in `onupgradeneeded` (guarded by `contains`) — separate from the `sw.js` `CACHE` version.
- **The status pill's activity log is diagnostic, and it is built so it can never cost the capture page anything.** Tapping the pill opens the Sync activity sheet (`js/activity-log.js`): the offline queue's items in send order, the running pill jobs, and ~2 days of history (48 h / 500 rows), each row expanding to every field sent (never the token). Three choices are load-bearing, and each looks like something to "tidy up":
  (1) **It is loaded with a dynamic `import()` inside a `.catch`, and it is deliberately NOT in `sw.js` SHELL.** Any byte change to `sw.js` — adding one SHELL entry included — installs a new worker whose `addAll(SHELL)` re-downloads the whole shell on every phone at its next open, which is exactly the fleet-wide effect the owner rejected with the `CACHE` bump (standing rule above). The fetch handler runtime-caches the file on its first successful load and stale-while-revalidate keeps it current; ⟳ Force update does not re-fetch it. A phone that can't load it loses the log and nothing else — a *static* import that failed to resolve would take down the whole capture module, Log stop and ⟳ Force update included. If `sw.js` is ever edited for another reason, adding the file to SHELL then is free.
  (2) **Its hooks never add a named import to the critical path.** Per-file skew means a new `capture.js` can run against an old `queue.js`/`dom.js`/`api.js`, and a missing named export is a link error that kills the page. So the queue feed rides the long-standing `setQueueHooks` (`onLog`, handed in by `capture.js`), and the two newer hooks — `dom.js onActivityEvent`/`activeActivities` and `api.js setApiHook` — are reached through `import * as dom/api` with a `typeof` check, so a stale file costs one feed. `queue.js` never imports the log; every event goes through one fenced `log(ev)` that can't throw into the drain. `tests/activity-log.test.mjs` pins all of this.
  (3) **It has its own IndexedDB database (`meterlog-activity`), not a store in `meterlog`.** A new store means a `DB_VERSION` bump, and `idb.js` has no `onblocked`: a v5 connection held by a second tab or `planner.html` would hang every `idb.*` call — the write inside `enqueue` included — until it closed. The system-of-record database stays at 5.
  Row model: **one row per queued write, updated in place** (`'q:'+_seq`: queued → waiting / rejected / stuck → sent), so an hour offline changes one row's status and adds nothing; one row per pill job, written when it ends, with a POST made inside exactly one open job folded into it (the worklist sync after every logged stop is one row, not two); repeats within 2 min collapse to ×N. A queue row whose item left the queue with no recorded result reads "Left the queue" rather than guessing Sent.
...
- **An element in a page's markup is a CROSS-FILE CONTRACT with the module that binds
  it — and `sw.js` can deliver the two halves one deploy apart.** The fetch handler is
  **stale-while-revalidate per FILE**: each shell file refreshes on its own schedule, so
  a phone legitimately runs a new `js/*.js` against the **previous** `index.html` for an
  open or two — indefinitely on a truck, where the background re-fetch just fails and
  keeps the old bytes. `$()` returns **null** for a missing id rather than throwing, so
  an unguarded `$('newThing').onclick = …` throws a TypeError, and **a module stops
  evaluating at its first uncaught exception**. And because the `CACHE` version is never
  bumped here (see the standing rule above — updates are manual, on the installer's tap),
  **there is no atomic-shell escape from this**: skew is not a brief window after a deploy,
  it is an ordinary long-lived state, and defensive binding is the whole defence rather
  than a stopgap. It cost a production day on **2026-08-14**:
  `js/worklist-tuning.js` bound the brand-new `#tuneNavByAddress` unconditionally, the
  commit reasoned "no new module or stylesheet, so no bump", and every phone that had
  taken the new JS but not the new HTML threw inside `initWorklist()` — which
  `js/pages/capture.js` calls at **module top level**, so ~40 handlers *below* that line
  were never bound. A headless repro measured **16 of 20 dead**, including `logStop`.
  The crew reported the one they happened to press: *"the End of day button does not
  work."* Worse, **`#refreshApp` — Settings ▸ ⟳ Force update, the only way code reaches
  these phones at all — is bound below that same line**, so an affected phone could not
  force its own repair: the documented recovery path was among the casualties. Three rules
  follow, and the first two are independent belts:
  (1) **bind a freshly-added id defensively** — `const el = $('x'); if(el) …`. Not for one
  release; for as long as any phone might still be on older markup, which with manual
  updates is indefinitely;
  (2) **`initWorklist()` in `capture.js` stays wrapped in try/catch.** The worklist screen
  is a *sibling* of the capture form, not a dependency of it: Log stop, the downtime form,
  Today, the PDF and the whole end-of-day close-out are wired below that call, and they are
  the safety-critical path. A broken worklist screen must cost the worklist screen only —
  degrade, never destroy, the same rule the duplicate-J warning follows. Contained is not
  hidden: it `console.error`s the real stack **and** shows the installer a notice;
  (3) **don't reason about the blast radius from the symptom.** One dead button and forty
  dead buttons look identical to a crew that only pressed one. `tests/shell-skew-survivable.test.mjs`
  pins all of this.
...
- **Route timing has two sheet-backed references, and times are road-only.** The desktop planner (and the phone) route toward each installer's **home** (`Employees.homeAddress`, the end-of-day bias) — the route runs furthest-meter-first, then works inward toward home. The crew's **team start** (`Teams.startAddress`, the 08:00–08:30 morning muster point) is **ETA-only: never an ordering anchor.** It stays in the distance/duration matrix so the drive OUT to the day's first (furthest) stop can be timed (the first work order's ETA = depart + that drive), priced (`homeLegMetersFor`), and drawn (the faint dashed drive-out line + start pin), but it does **not** pin where the route begins — `solveVariant` drops it from the ordering matrix when `startIsCommute` (a team start). Only a phone GPS "start from here" (`startFromCurrent`) still anchors the route at your current fix. Both are appended columns (`ensureTeamsColumns`/`ensureEmployeesColumns` create them on save; geocoded lazily by the planner/phone, not the admin page). ETAs are built from a **duration matrix `T`** (`measure.T`, `travelLookup`): `route-constraints.js` adds real travel + `onSiteMinutes(pace)` and `timeCapacity` sizes each day so the target lands by ~14:00 (fewer stops when travel is heavy). **Every source now carries `T`:** OSRM (planner) and — as of the durations work — **Google Routes** (`duration` in the FieldMask) and **ORS** (`metrics:['distance','duration']`) return real minutes, and when no road matrix is pulled (the phone's free straight-line default, or a fallback run) `optimizeRoute` **estimates `T` from the distances** (`estimateDurations`, crow-flies × `ROAD_DETOUR_FACTOR` ÷ `ESTIMATE_SPEED_KMH`) and flags `estimatedTimes`. So the phone finally shows crew-start ETAs + workday-sized days even offline — labeled "(est.)"/"~" in the route view. **The phone must pass `travel` (`travelLookup(measure)`, or `estimateTravelFromCoords` on the matrix-less flip/drag paths) into `scheduleRouteConstraints`** — without it the ETAs silently collapse to flat pace (`firstMin + i·pace`, the old "8:15 first stop, no drive-out" bug). (The `T`-in-minutes convention is uniform: OSRM/ORS ÷60 from seconds, Google via `parseGoogleDuration`.) The landing/"expected stops" projection is **one real-data model** (`js/compute/estimate.js` `projectDayReal` + `workHorizon`), shared by the **plan-mode banner** (`renderPlanEstimate`), the **Drive-screen on-pace line** (`drive.js paintPace`), and the **`#tuning` what-if** — so the three never disagree (the old blended-average `projectDay` is gone). It reprojects **today's remaining route** with travel and on-site split, both halves priced by **one** `estimateTravel` lookup built over today's whole route, done orders included (remaining travel = that ladder walked along the pending chain; on-site = today's observed WO→WO cadence with each gap's own measured drive netted out — *not* `legMetres ÷ avgMovingSpeed` and *not* `pace − 10`, see §"No day-average speed prices the route"), capped at the stops left, and returns **two paces** — the installer's **finish-by target** and **regular hours** (3:45, → 4:45 OT past 4:00 while the day is still open) — plus the top-level **route-finish clock** `routeFinishMin`/`routeFinishLabel` (`now + remaining travel + stops × on-site`, the same three terms `paceFor` inverts, read forward; horizon-independent, so it is deliberately *not* inside a pace). **Two paces in the model, but the Drive screen paints only the regular-hours card** — the target card's horizon is `ROUTE_DAY_END` (4:00), fifteen minutes from 3:45, and it read as the same card twice; `paces.target` stays for the plan banner and the tuning what-if. All three read `worklist.js paceContext`/`drivePace`. **Closing out the day** (`finishDay`) stamps `localStorage['dayClosedDate']` (kills the OT escalation) and exits plan mode (`exitPlan`). See ARCHITECTURE.md §"Drive mode".
- **Day 1 is frozen by the WORK-LIST LOCK — and don't day-chunk over raw `pending` on a passive path.** The multi-day split cuts the *pending* list into `target`-sized chunks, so finishing orders lets a re-chunk refill Day 1 from the front of what's left, pulling tomorrow's orders up into today. That is right while the installer is planning and wrong once they have decided what they are driving, so **they say which**: a 🔓/🔒 toggle above 🧭 Optimize (`#wlDayLock`) and in the planner's settings card (`#plDayLock`). `worklist.js applyTodayAnchor()` is still the single choke point (pure helpers in `route-today.js`); it now branches on `dayLocked()`. **The lock is ONE DATE** — `localStorage['wlDayLock']`, locked iff it equals the plan day — so a new plan day releases it for free, with nothing to expire. The frozen SET is `localStorage['wlTodayAnchor']` `{date, ids, target}`, written **once at the press** by `freshAnchorIds` (the route's current lowest-`day` group, i.e. what is on the screen the installer is looking at). Only "Add to today" ever grows it. It runs at every point that stamps `day` — after optimize (pass the **real** `travel` so ETAs stay exact), after Download, after `markWorklistDone`, after `resetWorklistOrder`, on first view, and after **every** logged stop via `planAdvance` — and writes a pending order only when its `order`/`day`/date/ETA actually change (so an exact downloaded ETA survives an unchanged day). If you add a new writer of `day`, route it through `applyTodayAnchor` or Day 1 will drift again. See ARCHITECTURE.md §"The work-list lock".
- **"Unlocked" does NOT mean "no `day1Count`", and getting that wrong re-ships the worst bug this codebase has had.** `applyTodayAnchor` runs after every logged stop, on the Drive screen's 3-minute `autoSync`, on Download and at boot. If those re-chunked by target, an installer who had simply not pressed 🔒 would watch tomorrow's orders climb into today as they worked — *"the next day's work orders are shuffling up every time … I say that I'm going to do X, and I want it to stay at that"* — on a timer. So there are **three** answers: **locked** ⇒ the frozen set entire; **unlocked + `opts.rechunk`** ⇒ `null`, size every day by the target; **unlocked + passive** ⇒ hold the day as currently tagged (`taggedDay1Ids`). Four things are load-bearing: (1) **exactly one caller passes `rechunk`** — the meters/day box, which moved the number without re-solving, so its `day` tags still describe the old one. Optimize must NOT pass it: it rewrites every tag at the new target moments before it calls, so holding them *is* the fresh split; (2) **`day1Count` 0 is a real value, not "unset"** — `route-constraints.js` tests it against `null`, because a falsy check hands a finished locked day a whole fresh target; (3) **every path that re-schedules the pending list must pass the count**, not just this one — `switchVariant` and `persistOrderIds` call `scheduleRouteConstraints` themselves, and while they omitted it the lock leaked (flip road↔straight, or drag one card, and Day 1 silently went back to a full target). `currentDay1Count()` is the single answer all three read; (4) **the target sizes the day when the day is PLANNED, then lets go** — installing past it reads as ahead (`targetOver` on the gauge, a later `Route done ~`), never as a shorter list.
- **The inference the toggle replaced is gone. Don't rebuild it under a new name.** The freeze used to be automatic, and all its difficulty was guessing *when the installer meant to re-plan*: `needsCommit` with four reasons, `dayCapacity` bounding a mid-day raise, `freshAnchorIds`' `opts.max`/`opts.fromTags`, and a `freeReplan`/`midReplan`/`exhausted` triangle in `applyTodayAnchor`. Every piece was a correct fix to a real report and the installer still could not answer *"is my day settled right now, and what do I press to change it?"* Two of its lessons survive as rules, because they are about the domain and not the mechanism: (1) **the target counts METERS and the set counts ORDERS** — Day 1 was once `min(dayCapacity(target, installedToday) + anchor.extend, |set ∩ pending|)`, so a two-meter order or any walk-up (which `markWorklistDone` never sees) spent the day's room faster than it spent the list and stamped a committed day's tail Day 2 by mid-morning; once the target was *met* with orders still pending, capacity hit 0 and **every** remaining order went to day 2+, taking the pace card off the screen with the crew still working. Nothing may re-size a locked day from meters installed; (2) **an exhausted locked day stays exhausted** — `needsCommit` used to re-commit the instant the set emptied, hauling the next chunk up. A finished day is finished; more work arrives by unlocking, or by hand through `#wlAddTo`. Two smaller rules ride along: **the toggle also takes the meters/day box and the plan-date picker read-only**, and `readOnly` is the *affordance* only — a number input's spinner and a date input's picker both ignore it on some browsers, so the `onchange` handlers are the actual guard; and **a phone mid-day when this shipped keeps its day** via `migrateLegacyLock()`, a one-shot keyed on its own marker rather than on "`wlDayLock` is blank" (blank is the real unlocked state, so migrating off it would re-lock on every reload after an unlock).
...
- **A GPS fix goes stale without ever going null, and that is the whole trap.**
  The day's remaining ETAs are anchored on the truck's live position
  (`js/worklist.js livePin` → `js/drive-recorder.js lastFix`), ahead of the old
  `lastDonePin` and the muster point in `estimateTravel`'s ladder. **A web app gets
  no GPS in the background** — so the moment the PWA is backgrounded, which is
  exactly what a Google-Maps hand-off does, `watchPosition` stops and `lastFix()`
  keeps returning the point where the driver *left*, indefinitely and with no error
  to notice. Unbounded, the "current location" origin is *most* wrong precisely
  while the driver is driving, which is the one moment it exists for.
  `LIVE_FIX_MAX_AGE_MS` (2 min) is what makes it decay back to `lastDonePin` instead;
  it is a correctness bound, not a tuning knob, and widening it buys nothing —
  ~3 km of error at highway speed already sits inside the ±30% `ROAD_DETOUR_FACTOR`
  carries, while five minutes would be ~7 km and would dominate the estimate. Four
  things ride along:
  (1) **`lastFix()` is a SIBLING of `liveMetrics()`, not a fifth field on it.** That
  return shape is a HUD contract `drive.js paintMetrics` and `worklist.js routeTravel`
  both destructure, and `tests/pace-live.test.mjs` pins. A position is not a metric;
  (2) **`routeTravel` must re-price the first pending leg from the same fix.** Its
  saved `legMeters` is the drive from the previous stop *in route order*, so anchoring
  ...
  matrix with;
  (4) **the fallback must stay silent.** A phone that never armed drive tracking
  reaches none of this and behaves exactly as it did before — no location prompt, no
  GPS wakeup of its own, nothing on screen. That was the explicit ask, and it is also
  what keeps `lastDonePin`'s original virtue (free, no permission, works with the
  radio off) available on the capture phone, which usually *is* standing in the
  driveway it last logged.
  The Drive card's arrival line reads the persisted `scheduledEta` rather than
  computing its own drive time, for the same one-model reason, and its "in N min" goes
  through `hhmmMin(clockOf(stamp()))` — `scheduledEta` is a **Toronto** clock string,
  so a bare `new Date().getHours()` compares two zones and fails invisibly on a phone
  set to Toronto (found only by driving the real screen in a UTC container). See
  ARCHITECTURE.md §"Drive mode".
- **An unattended refresh is not a Download with the prompts removed.** The Drive
  screen refreshes itself from the sheet every 3 minutes while the driver is
  ...
  every few minutes through a dead zone is worse than the staleness it reports;
