...
- **A field the route reads live must be persisted live — and storing it is only half
  ...
  The second half is the target box, and it was **asymmetric**. Day 1 is sized
  `day1Count(anchor, day1Ids, dayCapacity(target, installedToday))` =
  `min(capacity + anchor.extend, anchor.ids.length)`. **Lowering** the target clamps the
  day down immediately through `capacity`; **raising** it did nothing at all, because the
  frozen anchor set is re-committed only when `needsCommit` sees `replan` *and* a changed
  target — and only an explicit Optimize passed `replan`. "They get stuck on a different
  value and never got updated" was literally true for every increase. `wlTarget`'s
  ...
  bounding a started day to the `dayCapacity` it has left, and the `capacity === 0` skip
  that refuses to commit an empty set. It passes `replan` **without** `travel`, so the
